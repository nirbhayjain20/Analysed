...
if ($_POST) {
 ...
 if ($name=='' && $position=='' && $task=='' && $location=='') {
 		$queryMeal = "SELECT * FROM tasklisting WHERE c_date BETWEEN CURDATE() AND CURDATE()+10;";
 	}else{
        ...
        if ($task!='') {
 		...
 		$queryMeal = "SELECT * FROM tasklisting WHERE $nameSearch AND $positionSearch AND $taskSearch AND $locationSearch AND c_date BETWEEN CURDATE() AND CURDATE()+10";
 	}

...
if ($_POST) {
 ...
 if ($name=='' && $position=='' && $task=='' && $location=='') {
 		$queryMeal = "SELECT * FROM tasklisting WHERE c_date=CURDATE();";
 	}else{
        ...
        if ($location!='') {
 		...
 		$queryMeal = "SELECT * FROM tasklisting WHERE $nameSearch AND $positionSearch AND $taskSearch AND $locationSearch AND c_date=CURDATE()";
 	}
